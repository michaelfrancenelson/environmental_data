...
all.subsets.glm <-
  function(y, x1 = NULL, x2 = NULL, x2.linear = FALSE, x.force = NULL, 
    ...
    if(model.ave == TRUE){
      ...
      subset.model.ave.results<-as.data.frame(temp$coefmat.subset)
      full.model.ave.results<-as.data.frame(temp$coefmat.full)
    }
...
ci.lines <-
  function(model, interval = 95, type = 'expected', ...){
    xm<-mean(model$model[, 2])
    n<-length(model$model[, 2])
    ssx<-sum(model$model[, 2]^2)-sum(model$model[, 2])^2/n
    prob<-1-((1-(interval/100))/2)
    s.t<-qt(prob, (n-2))
    xv<-seq(min(model$model[, 2]), max(model$model[, 2]), (max(model$model[, 2])-min(model$model[, 2]))/100)
    yv<-coef(model)[1]+coef(model)[2]*xv
    if(type == 'expected') se<-sqrt(summary(model)$sigma^2*(1/n+((xv-xm)^2/ssx)))
    else if(type == 'predicted') se<-sqrt(summary(model)$sigma^2*(1+(1/n)+((xv-xm)^2/ssx)))
...
concordance <-
  function(use.data, avail.data, formula, method = 'glm', weights = NULL, 
           paired = FALSE, use.strata = NULL, avail.strata = NULL, v = 10, bins = NULL, balanced = TRUE, 
           max.p = NULL, 
    ...
    cc.bins<-function(bins, max.p){
      ...
      if(is.null(max.p)) max.p <- 1
      bin.mids<-seq(max.p/(bins*2), max.p-(max.p/(bins*2)), by = max.p/(bins))
      bin.ranges<-seq(0, max.p, by = max.p/bins)
    ...
    if(paired == TRUE){
      ...
      for(i in 1:v){ 
        ...
        if(!is.null(weights)){
          if(method == 'glm') fit<-glm(formula, data = train.use, family = 'binomial', weights = weights)  
          else if(method == 'lmer' | method == 'glmer') fit<-glmer(formula, data = train.use, family = 'binomial', weights = weights)
        } else {
      ...
      for(i in 1:v){ 
        ...
        if(v>1){
          ...
          if(!is.null(weights)){
            if(weights == TRUE){
              train.use$Weights <- 1
              train.avail$Weights <- nrow(train.use)/nrow(train.avail) 
            } else {
              if(balanced == TRUE){
                train.avail[, weights] <- sum(train.use[, weights])/nrow(train.avail)
              } 
              colnames(train.use)[which(colnames(train.use) == weights)] <- "Weights"
              colnames(train.avail)[which(colnames(train.avail) == weights)] <- "Weights"
            }
          }
        ...
        #fit model to training data subset
        if(!is.null(weights)){
          if(method == 'glm') fit<-glm(formula, data = train, family = 'binomial', weights = Weights)  
          else if(method == 'lmer' | method == 'glmer') fit<-glmer(formula, data = train, family = 'binomial', weights = Weights)
        } else {
    ...
    if(!is.null(max.p)) max.p <- max(valid.out, avail.out)
    ...
    if(is.null(bins)){
      ...
      for(i in 1:length(bin.seq)){
        cc.results[[i]]<-cc.bins(bins = bin.seq[i], max.p = max.p)
    ...
    #user specified bin number
      ...
      z<-cc.bins(bins = bins, max.p = max.p)  
    }
